...
(function(){
  function patchHome(){
    ...
    input.id='birthDate'; input.type='hidden';
    ...
    function applyBirthDate(dateValue){
      const parts=dateValue.split('-').map(Number); if(parts.length!==3||parts.some(Number.isNaN))return;
      const [year,month,day]=parts,animal=animals[(year-2020+120)%12],now=new Date();
    ...
    const title=picker.querySelector('.bp-title'),days=picker.querySelector('.bp-days');
    ...
    function renderCalendar(){
      ...
      for(let d=1;d<=last;d++){
        ...
        const dt=new Date(viewYear,viewMonth,d);btn.disabled=dt>maxDate||viewYear<minYear;
        if(input.value===k)btn.classList.add('selected'); if(dt.toDateString()===today.toDateString())btn.classList.add('today');
        btn.addEventListener('click',e=>{e.stopPropagation();input.value=k;localStorage.setItem('mira_birth_date',k);renderDate();applyBirthDate(k);closePicker();});days.appendChild(btn);
      }
    ...
    // 한 번 클릭하면 열고, 같은 박스를 다시 클릭하면 닫습니다.
    ...
    picker.querySelector('.bp-prev').addEventListener('click',e=>{e.stopPropagation();if(viewMonth===0){viewYear--;viewMonth=11;}else viewMonth--;renderCalendar();});
    picker.querySelector('.bp-next').addEventListener('click',e=>{e.stopPropagation();if(viewMonth===11){viewYear++;viewMonth=0;}else viewMonth++;renderCalendar();});
    ...
    form.addEventListener('submit',e=>{e.preventDefault();if(!input.value){result.textContent='생년월일을 선택해주세요.';openPicker();return;}localStorage.setItem('mira_birth_date',input.value);renderDate();applyBirthDate(input.value);closePicker();});
...
// 날짜 입력창과 커스텀 달력을 깔끔하게 표시합니다.
(function(){
  const style=document.createElement('style');style.textContent=`
    .birth-date-trigger{position:relative;width:230px;height:48px;display:flex;align-items:center;padding:0 14px;background:#f8f5ed;border:1px solid #d7d0c1;border-radius:12px;color:#89928a;cursor:pointer;overflow:hidden;box-sizing:border-box;user-select:none}
    .birth-date-trigger:hover{border-color:#a9bda9;background:#fbf9f3}.birth-date-trigger:focus{outline:none;border-color:#5c8d71;box-shadow:0 0 0 4px rgba(92,141,113,.12);background:#fffdf8}
    .birth-date-icon{width:28px;flex:0 0 28px;color:#aa8f59;font-size:13px;pointer-events:none}.birth-date-value{font-size:14px;font-weight:700;pointer-events:none}.birth-date-value.has-value{color:#244638}
    .birth-picker{position:fixed;z-index:99999;width:280px;padding:14px;background:#fffdf8;border:1px solid #ddd6c7;border-radius:16px;box-shadow:0 16px 40px rgba(36,70,56,.16);display:none}.birth-picker.open{display:block}
    .birth-picker-head{display:grid;grid-template-columns:36px 1fr 36px;align-items:center;margin-bottom:10px}.birth-picker-head b{text-align:center;font-size:14px;color:#244638}.birth-picker-head button{width:32px;height:32px;border:0;border-radius:9px;background:#f1f4ee;color:#507960;font-size:22px;cursor:pointer}.birth-picker-head button:hover{background:#e4eee5}.birth-picker-head button:disabled{opacity:.3;cursor:default}
    .bp-week,.bp-days{display:grid;grid-template-columns:repeat(7,1fr);gap:3px}.bp-week span{text-align:center;font-size:10px;color:#999f98;padding:5px 0}.bp-days{margin-top:3px}.bp-day,.bp-empty{width:32px;height:32px;display:flex;align-items:center;justify-content:center;border:0;border-radius:9px;background:transparent;font-size:11px;color:#52675a;cursor:pointer}.bp-day:hover{background:#eaf2eb;color:#244638}.bp-day.today{box-shadow:inset 0 0 0 1px #b9ccb9}.bp-day.selected{background:#5c8d71;color:#fff;font-weight:900}.bp-day:disabled{color:#d4d2ca;cursor:default;background:transparent}
