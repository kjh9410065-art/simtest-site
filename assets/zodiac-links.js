...
(function(){
  function patchHome(){
    ...
    function openPicker(){positionPicker();renderCalendar();picker.classList.add('open');}function closePicker(){picker.classList.remove('open');yearMode=false;}
    ...
    document.addEventListener('click',e=>{if(!trigger.contains(e.target)&&!picker.contains(e.target))closePicker();});window.addEventListener('resize',()=>{if(picker.classList.contains('open'))positionPicker();});
    form.addEventListener('submit',e=>{
      e.preventDefault();
      if(!input.value){result.textContent='생년월일을 먼저 설정해주세요.';result.classList.add('birth-warning');openPicker();return;}
    ...
    resetBtn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();input.value='';localStorage.removeItem('mira_birth_date');renderDate();result.textContent='';result.classList.remove('birth-warning');renderEmptyState();closePicker();});
    renderEmptyState();
  }
...
/* 경고 문구는 입력칸의 왼쪽에 고정해 표시하여 입력칸 위치가 밀리지 않게 합니다. */
.birth-form{position:relative}.birth-result.birth-warning{position:absolute;right:calc(100% + 10px);top:50%;transform:translateY(-50%);margin:0;white-space:nowrap;z-index:2}
@media(max-width:760px) and (hover:none) and (pointer:coarse){.birth-date-trigger{width:auto;flex:1;height:46px}.birth-date-icon{width:26px;flex-basis:26px}.birth-picker{width:calc(100vw - 24px);max-width:320px}.bp-day,.bp-empty{width:100%;height:34px}.birth-form{display:flex;flex-wrap:wrap}.birth-form .birth-date-trigger{flex:1;min-width:0}.birth-reset{height:38px}.birth-result.birth-warning{position:absolute;left:0;right:auto;top:calc(100% + 8px);transform:none;margin:0}#fortune .overview>.main,#fortune .overview>.lucky{height:232px;min-height:232px}#fortune .four{grid-template-columns:repeat(2,1fr)}#fortune .four>.fortune{height:116px;min-height:116px}}
`;
...
(function(){
  function bind(){
    ...
    form.addEventListener('submit',function(){
      setTimeout(function(){
        const input=document.getElementById('birthDate');if(!input||!input.value)return;
        const p=input.value.split('-').map(Number);if(p.length!==3||p.some(Number.isNaN))return;
        const [y,m,d]=p;const seed=(y*31+m*17+d*13+new Date().getDate()*7)%6;
        const scoreEl=document.getElementById('score');const score=scoreEl?Number(scoreEl.textContent)||76:76;
        ...
        const texts=[money,love,work,health];const cards=document.querySelectorAll('#fortune .four .fortune');
        cards.forEach((card,i)=>{const value=Math.max(72,Math.min(96,score+((seed+i*2)%7)-3));const strong=card.querySelector('strong'),small=card.querySelector('small');if(strong)strong.textContent=value+'점 · '+grades[(seed+i)%grades.length];if(small)small.textContent=texts[i][(seed+i)%texts[i].length];});
      },0);
    });
    const reset=form.querySelector('.birth-reset');
    if(reset)reset.addEventListener('click',function(){setTimeout(function(){const rows=document.querySelectorAll('#fortune .lucky .luckrow b');rows.forEach((el,i)=>el.textContent=i===0?'설정 후 확인':'—');document.querySelectorAll('#fortune .four .fortune').forEach(card=>{const strong=card.querySelector('strong'),small=card.querySelector('small');if(strong)strong.textContent='—';if(small)small.textContent='생년월일을 설정하면 확인할 수 있어요.';});},0);});
  }
