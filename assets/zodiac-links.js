...
(function(){
  function patchHome(){
    ...
    const mainTitle=document.querySelector('#fortune .main h3');
    if(birthCopy)birthCopy.textContent='생년월일을 설정한 후 운세 보기 버튼을 눌러주세요.';
    if(mainTitle)mainTitle.textContent='오늘의 운세';
    ...
    // 저장된 날짜가 있어도 버튼을 누르기 전에는 결과 영역을 숨깁니다.
    if(saved)renderDate();

    // 설정 완료 후 '운세 보기' 버튼을 눌렀을 때만 결과 영역을 공개합니다.
    function showFortune(){const overview=document.querySelector('#fortune .overview');const four=document.querySelector('#fortune .four');if(overview)overview.classList.add('fortune-visible');if(four)four.classList.add('fortune-visible');}
...
/* 생년월일을 설정하기 전에는 오늘의 운세 결과 카드 전체를 빈 상태로 둡니다. */
