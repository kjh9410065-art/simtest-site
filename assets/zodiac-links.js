...
(function(){
  function patchHome(){
    ...
    function renderCalendar(){
      yearMode=false;yearPanel.classList.remove('open');picker.querySelector('.bp-week').style.display='grid';days.style.display='grid';
    ...
    // 날짜 선택 버튼 옆에 저장된 생년월일을 지우는 초기화 버튼을 추가합니다.
    const resetButton=document.createElement('button');
    resetButton.type='button';
    resetButton.className='birth-reset';
    resetButton.textContent='초기화';
    form.insertBefore(resetButton,form.querySelector('button[type="submit"]'));

    // 초기화하면 저장값과 화면의 개인 운세 상태를 모두 원래 상태로 되돌립니다.
    resetButton.addEventListener('click',e=>{
      ...
      renderDate();
      closePicker();
      result.textContent='생년월일을 설정한 후 운세 보기 버튼을 눌러주세요.';
      if(badge)badge.innerHTML='오늘의 운세';
      if(summaryTitle)summaryTitle.textContent='생년월일을 설정한 후 운세를 확인해보세요.';
    ...
    // 저장된 생년월일이 있어도 버튼을 누르기 전에는 오늘의 운세 결과를 보여주지 않습니다.
    if(saved)renderDate();
    ...
    function showFortune(){
      ...
      if(overview)overview.classList.add('fortune-visible');
      if(four)four.classList.add('fortune-visible');
    }
    const overview=document.querySelector('#fortune .overview'),four=document.querySelector('#fortune .four');
    if(overview)overview.classList.remove('fortune-visible');
    if(four)four.classList.remove('fortune-visible');
...
// 날짜 선택 UI와 초기화 버튼을 깔끔하게 표시합니다.
(function(){const style=document.createElement('style');style.textContent=`
.birth-date-trigger{position:relative;width:230px;height:48px;display:flex;align-items:center;padding:0 14px;background:#f8f5ed;border:1px solid #d7d0c1;border-radius:12px;color:#89928a;cursor:pointer;overflow:hidden;box-sizing:border-box;user-select:none}.birth-date-trigger:hover{border-color:#a9bda9;background:#fbf9f3}.birth-date-trigger:focus{outline:none;border-color:#5c8d71;box-shadow:0 0 0 4px rgba(92,141,113,.12);background:#fffdf8}.birth-date-icon{width:28px;flex:0 0 28px;color:#aa8f59;font-size:13px;pointer-events:none}.birth-date-value{font-size:14px;font-weight:700;pointer-events:none}.birth-date-value.has-value{color:#244638}
.birth-reset{height:38px;border:1px solid #d7d1c5;border-radius:9px;background:#fbf9f2;color:#7a837c;padding:0 12px;font-size:11px;font-weight:900;cursor:pointer}.birth-reset:hover{background:#f1ece1;border-color:#c9c1b1;color:#52675a}
...
/* 생년월일을 설정하기 전에는 오늘의 운세 결과를 숨겨 안내만 보여줍니다. */
#fortune .overview,#fortune .four{display:none}
#fortune .overview.fortune-visible{display:grid}
#fortune .four.fortune-visible{display:grid}
@media(max-width:760px) and (hover:none) and (pointer:coarse){.birth-date-trigger{width:auto;flex:1;height:46px}.birth-date-icon{width:26px;flex-basis:26px}.birth-picker{width:calc(100vw - 24px);max-width:320px}.bp-day,.bp-empty{width:100%;height:34px}.birth-form{display:flex;flex-wrap:wrap}.birth-reset{height:38px}.birth-result{width:100%}}
