/* 한 문제씩 보여주는 공통 심리테스트 엔진입니다. 추가 테스트 데이터도 함께 사용합니다. */
const ALL_TEST_DATA=Object.assign({},TEST_DATA,typeof EXTRA_TEST_DATA!=='undefined'?EXTRA_TEST_DATA:{});const d=ALL_TEST_DATA[TEST_ID];let n=0,answers=[];const $=id=>document.getElementById(id);
...
function render(){
 ...
 $('prev').disabled=n===0;$('next').textContent=n===d.questions.length-1?'결과 보기':'다음';
}
...
function showResult(){
 const counts=[0,0,0,0],sum=answers.reduce((a,b)=>a+b,0);answers.forEach(v=>counts[v]++);const type=counts.indexOf(Math.max(...counts));const r=d.types[(type+sum)%d.types.length];
