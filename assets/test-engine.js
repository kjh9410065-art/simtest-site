/* 한 문제씩 보여주는 공통 테스트 엔진입니다. 결과는 선택 점수에 따라 해당 테스트의 고유 유형을 보여줍니다. */
const d=TEST_DATA[TEST_ID];let n=0,answers=[];
...
function showResult(){
 const counts=[0,0,0,0];answers.forEach(v=>counts[v]++);let type=counts.indexOf(Math.max(...counts));
 // 같은 점수라면 답변 전체를 이용해 결과가 한쪽으로 고정되지 않도록 합니다.
 const r=d.types[(type+answers.reduce((a,b)=>a+b,0))%d.types.length];
